...
function OpenedMessage({ chatDetails }) {
  ...
  useEffect(
    () =>
      onSnapshot(
        query(collection(db, `users/${auth.currentUser.uid}/chats/${chatDetails.with}-chat/messages`), orderBy("timeSent")),
        (snapshot) => {
  ...
  const sendMessage = async () => {
    await setDoc(doc(db, `users/${auth.currentUser.uid}/chats`, `${chatDetails.with}-chat`),{
      isSeen: false,
      recentMessage: message,
      timeSent: serverTimestamp(),
      with: chatDetails.with
    })
    await setDoc(doc(db, `users/${chatDetails.with}/chats`, `${auth.currentUser.uid}-chat`),{
      isSeen: false,
      recentMessage: message,
      timeSent: serverTimestamp(),
    ...
    //     collection(
    //       db,
    //       `users/${chatDetails.with}/chats/${auth.currentUser.uid}-chat`
    //     )
    //   ),
    //   {
    //     isSeen: false,
  ...
  const printDate = (str) => {
    return str == null ? "" : str.toDate().toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })
  }
  return (
    <div className="flex flex-col justify-end h-full mx-5">
      <div className="flex items-center shadow-b-md" style={{boxShadow: "0px 3px 0px #f0f0f0"}}>
        <Image src={recipient.profilePic} alt="profile" height={50} width={50} className="h-10 cursor-pointer rounded-full" />
        <div className="ml-3">
          {recipient.name}
        </div>
      </div>
      <div className="overflow-y-scroll scrollbar h-[45vh]" id="chatscroll">
        {chatList.map(
          (msg)=>{
            return <>
              <div className={`flex  my-1 ${msg.data().from ? "": "flex-row-reverse"}`}>
                <div className="relative cursor-pointer">
                  <Image src={msg.data().from ? recipient.profilePic : user.photoURL} alt="profile" height={50} width={50} className="h-10 cursor-pointer rounded-full" />
                </div>
                <div className={`h-fit flex items-end justify-between mx-3 py-1 px-2 shadow-md min-w-[50px] max-w-md rounded-lg ${msg.data().from ? "rounded-tl-none bg-white": "rounded-tr-none bg-[#F3F3F3]"}`}>
                  <div className="px-2">{msg.data().message}</div>
                  <div className='text-xs text-end min-w-fit' >{printDate(msg.data().timeSent)}</div>
                </div>
              </div>
            </>
          }
